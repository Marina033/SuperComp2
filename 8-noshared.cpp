...
int main(int argc,char *argv[])
{
    ...
    #pragma omp parallel for shared(a)  //private(i,j,x,el,ya,yb,l)
    for (int i=1; i<=M; i++)
    {
    ...
    for (it=0;it<1000000; it++)
    {
		...
		for (int k=pid; k<K; k +=nproc)
		{
			#pragma omp parallel for shared(w,dwthr)
			for (int i=ix1[k]; i<ix2[k]; i++)
			{
    ...
    Jacobi_time = MPI_Wtime() - Jacobi_time;
